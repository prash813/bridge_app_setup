...
def party():
    ...
    # play the music
    musicProc = subprocess.Popen(["mpg123", party['audioFile']])
...
def attributeChangeCallback(
    ...
    print("[callback] endpoint={} cluster={} attr={} value={}".format(endpoint, clusterId, attributeId, list(value)))
    ...
    for id, device in endpoints.items():
        ...
        match type:
            case 'bulb' | 'plug':
                ...
                match model:
                    case 'L530':
                        endpoints[id]['driver'] = PyL530.L530(ip, user, password)
                    case 'P100/P105':
                        endpoints[id]['driver'] = PyP100.P100(ip, user, password)
                    case 'P110/P115':
                        endpoints[id]['driver'] = PyP110.P110(ip, user, password)
                    case other:
